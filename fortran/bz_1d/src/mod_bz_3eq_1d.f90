...
real(kind=dp), private, parameter :: mu = 5.d-05
...
function f_bz_3eq_reac(n, u)
  implicit none
  integer :: n
  real(kind=dp), dimension(n) :: u
  real(kind=dp), dimension(n) :: f_bz_3eq_reac
  ! local
  real(kind=dp) :: ai
  real(kind=dp) :: bi
  real(kind=dp) :: ci
  ...
  do inx = 1, nx
   ...
   irow = 1 + (inx-1)*neq
    ...
    f_bz_3eq_reac(irow)   = oneovermu  * (-q*ai - ai*bi + f*ci)
    f_bz_3eq_reac(irow+1) = oneovereps * ( q*ai - ai*bi + bi - bi*bi)
    f_bz_3eq_reac(irow+2) = (bi - ci)

  end do 

end function f_bz_3eq_reac
...
subroutine build_bz_3eq_diff_matrix(a)
  implicit none
  type(coo_matrix) :: a
  ! local variables
  integer :: ntot, nnz
...
end subroutine build_bz_3eq_diff_matrix
